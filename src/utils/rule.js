...
export const formRule = {
    ...
    isJudgmentPassword: (rule, value, callback) => {
        if (!value) {
            callback(new Error('密码不能为空'))
        } else {
            if (includeSpaces(value)) {
                if (judgmentPassword(value)) {
                    callback()
                }
                callback(new Error('密码中必须包含字母、数字，长度在6-20位'))
            }
    ...
    userName: (rule, value, callback) => {
        if (!value) {
            ...
            if (enOrNum(value)) {
                if (rangeLength(value, [4, 20])) {
                    callback()
                }
                callback(new Error('用户名长度在4-20位'))

            }
            callback(new Error('用户名只允许输入字母或数字'))
        }
    }
}
 ...
 * 判断密码至少包含2种字符串类型
 */
export function judgmentPassword(value) {
    return new RegExp('(?=.*[0-9])(?=.*[a-zA-Z]).{6,20}').test(value)
}

/**
 * 判断字符串中是否包含特殊字符和空格
 */
export function includeSpaces(value) {
    return new RegExp(/^[\u4e00-\u9fa5_a-zA-Z0-9]+$/).test(value)
}